...
static void ft_syntax_err(char *str)
{
	...
	{
		if(str[1] == '<')
        	exit_status("minishell: syntax error near unexpected token `", str + 1, 1);
		if(str[0] == str[1] && str[1] == str[2])
        	exit_status("minishell: syntax error near unexpected token `", str + 2, 1);
	}
	if(str[0] == '<')
	{
		if((str[1] == '>' || str[0] == str[1]) && str[1] == str[2])
        	exit_status("minishell: syntax error near unexpected token `", "newline", 2);
